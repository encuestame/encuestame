...
import org.encuestame.core.social.SocialProviderOperations;
 ...
 * @version $Id:$
 */
public abstract class AbstractSocialSignInConnect<S> implements SocialProviderOperations<S>{
     ...
     * @return
     */
    public OAuth1Token fetchNewRequestToken(final String callbackUrl) {
        final Token requestToken = getOAuthService(callbackUrl).getRequestToken();
        return new OAuth1Token(requestToken.getToken(), requestToken.getSecret());
    }
    ...
    protected abstract S createServiceOperations(OAuth1Token accessToken);
     ...
     * @return
     */
    private OAuth1Token getAccessToken(final AuthorizedRequestToken requestToken) {
        ...
        return new OAuth1Token(accessToken.getToken(), accessToken.getSecret());
    }
