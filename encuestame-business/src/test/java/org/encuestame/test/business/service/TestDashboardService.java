...
public class TestDashboardService extends AbstractSpringSecurityContext{
     ...
     */
    @Test
    public void testgetAllGadgetsAvailable() throws EnMeNoResultsFoundException{
        getDashboardService().addGadgetOnDashboard(this.dashboard.getBoardId(), "stream");
        getDashboardService().addGadgetOnDashboard(this.dashboard.getBoardId(), "stream");
        getDashboardService().addGadgetOnDashboard(this.dashboard.getBoardId(), "stream");
        getDashboardService().addGadgetOnDashboard(this.dashboard.getBoardId(), "stream");
        getDashboardService().addGadgetOnDashboard(this.dashboard.getBoardId(), "stream");
        final List<GadgetBean> d = getDashboardService().getAllGadgetsAvailable(this.dashboard.getBoardId());
        assertEquals("Should be equals", 8, d.size());
    }
