...
@Controller
public class MySpaceConnectSocialAccount extends AbstractSocialController {
     ...
     * Consumer Key.
     */
    public @Value("${myspace.consumer.key}")
    String apiKey;

    /**
     * Consumer Secret.
     */
    public @Value("${myspace.consumer.secret}")
    String consumerSecret;

    /**
     * Authorize Url.
     */
    public @Value("${myspace.authorizeUrl}")
    String authorizeUrl;

    /**
     * Request Token Url.
     */
    public @Value("${myspace.requestToken}") String requestTokenUrl;

    /**
     * Access Token.
     */
    public @Value("${myspace.accessToken}") String accessToken;
    ...
    public String signinTwitterGet(
        ...
        OAuth1Token requestToken = tm
                .fetchNewRequestToken("http://localhost:8080/encuestame/social/back/myspace");
        request.setAttribute(OAUTH_TOKEN_ATTRIBUTE, requestToken,
                WebRequest.SCOPE_SESSION);
        String d = tm.buildAuthorizeUrl(requestToken.getValue(),
        "http://localhost:8080/encuestame/social/back/myspace");
        System.out.println("MYSPACE URL "+d);
        return "redirect:"+d;
    }
