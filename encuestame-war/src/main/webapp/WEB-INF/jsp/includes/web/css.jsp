<link rel="stylesheet" type="text/css" media="screen" href="<c:url value="/resources/css/web/default.css" />" />
<link rel="stylesheet" type="text/css" media="screen" href="<c:url value="/resources/css/web/user.css" />" />
<link rel="stylesheet" type="text/css" media="screen" href="<c:url value="/resources/css/web/public.css" />" />
<link rel="stylesheet" type="text/css" media="screen" href="<c:url value="/resources/css/web/menu.css" />" />
<link rel="stylesheet" type="text/css" media="screen" href="<c:url value="/resources/css/web/admon.css" />" />
<link rel="stylesheet" type="text/css" media="screen" href="<c:url value="/resources/static/web.css" />" />
<link rel="stylesheet" type="text/css" media="screen" href="<c:url value="/resources/js/dijit/themes/claro/claro.css"/>" />
<link rel="stylesheet" type="text/css" media="screen" href="<c:url value="/resources/js/dojox/form/resources/Rating.css"/>" />
<link rel="stylesheet" type="text/css" media="screen" href="<c:url value="/resources/js/dojo/resources/dojo.css" />" />